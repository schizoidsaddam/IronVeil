...
impl WorldTick {
    pub fn new() -> Self {
        Self { tick_count: 0, quake_cooldown: 0, last_heat_log: 0 }
    }
    ...
    fn age_world(
        ...
        match metrics.tension {
            ...
            WorldTension::Crisis => {
                ...
                // Sustained crisis pressure
                db.conn.execute(
                    "UPDATE factions SET
                        strength   = MIN(100, strength + 2),
                        aggression = MIN(100, aggression + 5)
    ...
    fn process_famine(
        ...
        // Famine grows when trade routes are disrupted + thermal stress
        // Famine > 60 spawns a famine event + raises revolt risk
        ...
        for (pid, pname, famine_level) in famine_provinces {
            ...
            // Famine spikes revolt risk hard
            db.conn.execute(
                "UPDATE provinces SET revolt_risk = MIN(100, revolt_risk + 20) WHERE id = ?1",
                params![pid],
            )?;

            // Famine slowly decays if not reinforced
            db.conn.execute(
                "UPDATE provinces SET famine = MAX(0, famine - 5) WHERE id = ?1",
                params![pid],
            )?;

            // Spawn rumor about famine
            Self::spawn_rumor(db, state.world_day, pid,
        ...
        // Natural famine decay in non-crisis provinces
        db.conn.execute(
            "UPDATE provinces SET famine = MAX(0, famine - 2) WHERE famine > 0",
            [],
        )?;
